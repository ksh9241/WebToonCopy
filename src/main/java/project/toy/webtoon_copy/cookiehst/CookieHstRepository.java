...
public interface CookieHstRepository extends JpaRepository<CookieHst, Long> {
    ...
    @Query(value = "SELECT * FROM cookie_hst A, cookie B, users C WHERE A.cookie_hst_seq = :cookieHstSeq AND A.cookie_seq = B.cookie_seq AND B.user_user_seq = C.user_seq", nativeQuery = true)
